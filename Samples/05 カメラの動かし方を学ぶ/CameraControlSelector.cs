...
namespace _05_HowToUpdateCamera
{
	public partial class CameraControlSelector : Form
	{
		...
		private void vmd_camera_controller_Click( object sender, EventArgs e )
		{
			...
			{
				var prov = VMDカメラモーション.ファイルから読み込む( ofd.FileName );
				prov.モーションを再生する();
		...
		{
			ResultCameraMotionProvider = new ボーン追従カメラモーション( new WeakReference<PMXModel>( Model ), "頭", 10f, new Vector3( 0, 0, 1 ), false );
			Close();
			/*

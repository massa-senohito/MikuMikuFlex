...
namespace _06_MoveBoneAndMorphFromCode
{
	public partial class TransformController : Form
	{
		...
		public TransformController( PMXModel model ) : this()
		{
			...
			foreach( var boneData in model.モデル.ボーンリスト )
				bone_combo_box.Items.Add( boneData.ボーン名 );

			foreach( var morphData in model.モデル.モーフリスト )
				morph_combo_box.Items.Add( morphData.モーフ名 );
			...
			updater = new 手動によるFK変形更新( new WeakReference<PMXModel>( model ) );

			//②-C ②-Bで作成したクラスを変形順序内に入れる。
			model.スキニング.変形更新リスト[ 0 ] = updater;
			/*
		...
		private void bone_x_plus_Click( object sender, EventArgs e )
		{
			...
			ボーン変形 transformer = updater.ボーン変形を検索して返す( (string) bone_combo_box.SelectedItem );
			transformer.回転 *= Quaternion.RotationAxis( new Vector3( 1, 0, 0 ), rotationAngle );
		}
		...
		private void bone_y_plus_Click( object sender, EventArgs e )
		{
			if( string.IsNullOrEmpty( (string) bone_combo_box.SelectedItem ) ) return;
			ボーン変形 transformer = updater.ボーン変形を検索して返す( (string) bone_combo_box.SelectedItem );
			transformer.回転 *= Quaternion.RotationAxis( new Vector3( 0, 1, 0 ), rotationAngle );
		}
		...
		private void bone_z_plus_Click( object sender, EventArgs e )
		{
			if( string.IsNullOrEmpty( (string) bone_combo_box.SelectedItem ) ) return;
			ボーン変形 transformer = updater.ボーン変形を検索して返す( (string) bone_combo_box.SelectedItem );
			transformer.回転 *= Quaternion.RotationAxis( new Vector3( 0, 0, 1 ), rotationAngle );
		}
		...
		private void morph_track_bar_ValueChanged( object sender, EventArgs e )
		{
			...
			transformer.モーフ値 = (float) morph_track_bar.Value / morph_track_bar.Maximum;
		}

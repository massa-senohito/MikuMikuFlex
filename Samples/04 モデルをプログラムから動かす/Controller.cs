...
namespace _04_TransformModelFormCode
{
	...
	public partial class Controller : Form
	{
		...
		public PMXModel Model { get; set; } // このクラスでは Dispose しないこと。
		...
		{
			Model.モデル状態.位置 += new Vector3( translationLength, 0, 0 );
		}

		private void trans_y_plus_Click( object sender, EventArgs e )
		{
			Model.モデル状態.位置 += new Vector3( 0, translationLength, 0 );
		}

		private void trans_z_plus_Click( object sender, EventArgs e )
		{
			Model.モデル状態.位置 += new Vector3( 0, 0, translationLength );
		}
		...
		/*
         * 説明：
         * モデルに関する平行移動、回転、拡大はすべて モデル状態 プロパティを介して行われる。
         * モデル状態はPMXModelだけでなく描画可能なIDrawableインターフェースを実装するすべてのクラスに含まれるため、
         * 今後のチュートリアルに登場するグリッドやXファイルモデルなどについても同様に平行移動、回転、拡大が可能である。
         * 
         * 平行移動は モデル状態.位置 プロパティを利用する。これはワールド座標系による座標系です。
         * 平行移動の値はベクトルのためベクトルの知識があると、この部分の理解は容易です。
         * また、回転はクォータニオン、拡大はベクトルで管理しています。
         * 拡大については モデル状態.倍率 プロパティに対してX軸方向、Y軸方向、Z軸方向への拡大率を代入することで可能です。
		...
		{
			Model.モデル状態.回転 *= Quaternion.RotationAxis( new Vector3( 1, 0, 0 ), rotationAngle );
		}

		private void rotate_y_plus_Click( object sender, EventArgs e )
		{
			Model.モデル状態.回転 *= Quaternion.RotationAxis( new Vector3( 0, 1, 0 ), rotationAngle );
		}

		private void rotate_z_plus_Click( object sender, EventArgs e )
		{
			Model.モデル状態.回転 *= Quaternion.RotationAxis( new Vector3( 0, 0, 1 ), rotationAngle );
		}
		...
		/*
         * 説明:
         * 回転は平行移動と同様に モデル状態 プロパティを介して行われる。
         * 回転にはクォータニオン(４元数)と呼ばれる表現が使われていて、数学的知識が必要のように見えるが表面上だけでの理解は容易である。
         * SharpDX.Quaternion.RotationAxis() は、指定されたベクトルを軸として指定した回転量を持つクォータニオンを生成する。
         ...
         * そのため、PMXModel.モデル状態.回転 *= となっています。
         */
		#endregion
	}
}

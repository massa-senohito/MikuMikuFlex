...
namespace MikuMikuFlex.エフェクト変数管理
{
    ...
    ///         
    ///     ○使用例
    ///         float4 mouse_down : MIDDLEMOUSEDOWN;
    ///         static float2 pos = mouse_down.xy;
    ///         static bool is_pressed = ( mouse_down.z != 0 );
    /// </summary>
	internal sealed class MIDDLEMOUSEDOWN変数 : マウス変数
	{
		public override string セマンティクス => "MIDDLEMOUSEDOWN";
        ...
        {
            return new MIDDLEMOUSEDOWN変数();
        }

        public override void 変数を更新する( EffectVariable 変数, 変数更新時引数 引数 )
		{
			変数.AsVector().Set( RenderContext.Instance.パネル監視.MiddleMouseDown );
		}
	}
}

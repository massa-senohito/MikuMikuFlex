...
namespace MikuMikuFlex3
{
	...
	public class テクニック : IDisposable
	{
        ...
        public テクニック( Effect effect, EffectTechnique d3dEffectTechnique, int subsetCount )
        {
            this.D3DEffectTechnique = d3dEffectTechnique;

            this.描画するサブセットIDの集合 = new HashSet<int>();

            this.パスリスト = new Dictionary<string, パス>();

            if( !D3DEffectTechnique.IsValid )
                throw new Exception( string.Format( "テクニック「{0}」の検証に失敗しました。", D3DEffectTechnique.Description.Name ) );
            ...
            this.GetSubsets( D3DEffectTechnique, subsetCount );
        ...
        public void パスの適用と描画をパスの数だけ繰り返す<T>( DeviceContext d3ddc, Action<T> drawAction, T drawTarget )
        {
            foreach( パス pass in パスリスト.Values )
                pass.適用して描画する( d3ddc, drawAction, drawTarget );
        }
        ...
        {
            EffectVariable annotationVariable = technique.GetAnnotationByName( attrName );

            if( annotationVariable == null || !annotationVariable.IsValid )
                return "";

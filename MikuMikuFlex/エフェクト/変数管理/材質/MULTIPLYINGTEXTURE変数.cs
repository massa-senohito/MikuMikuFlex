...
{
	internal sealed class MULTIPLYINGTEXTURE変数 : 材質変数
	{
		public override string セマンティクス => "MULTIPLYINGTEXTURE";
		...
		{
			return new MULTIPLYINGTEXTURE変数();
		}

        public MULTIPLYINGTEXTURE変数()
        {
        }
		...
		{
            // ターゲットオブジェクトに依存しない
			変数.AsVector().Set( 引数.材質.テクスチャ乗算値 );
		}
	}
}

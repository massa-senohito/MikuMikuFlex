...
namespace MMF.エフェクト.変数管理.行列
{
    ...
    /// 　正しい値を参照できなくなる。
    /// </remarks>
    internal sealed class VIEWPROJECTION変数 : 行列変数
	{
        public override string セマンティクス => "VIEWPROJECTION";


        public VIEWPROJECTION変数()
        {
        }

        private VIEWPROJECTION変数( Object種別 Object ) 
            : base( Object )
		{
		}
        ...
        {
            return new VIEWPROJECTION変数( Object );
        }
        ...
        public override void 変数を更新する( EffectVariable 変数, 変数更新時引数 引数 )
		{
            switch( ターゲットオブジェクト )
            {
                case Object種別.カメラ:
                    行列を登録する(
                        RenderContext.Instance.行列管理.ビュー行列管理.ビュー行列 * RenderContext.Instance.行列管理.射影行列管理.射影行列,
                        変数 );
                    break;

...
namespace MMF.エフェクト.変数管理.行列
{
    ...
    /// 　正しい値を参照できなくなる。
    /// </remarks>
	internal sealed class WORLD変数 : 行列変数
	{
        public override string セマンティクス => "WORLD";
        ...
        public override void 変数を更新する( EffectVariable 変数, 変数更新時引数 引数 )
		{
            ...
            {
                case Object種別.カメラ:
                    行列を登録する( RenderContext.Instance.行列管理.ワールド行列管理.モデルのワールド変換行列を作成して返す( 引数.モデル ), 変数 );
                    break;

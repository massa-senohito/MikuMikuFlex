...
namespace MMF.エフェクト.変数管理.行列
{
    ...
    /// 　正しい値を参照できなくなる。
    /// </remarks>
	internal sealed class WORLDVIEWPROJECTION変数 : 行列変数
	{
        public override string セマンティクス => "WORLDVIEWPROJECTION";


        public WORLDVIEWPROJECTION変数()
        {
        }
        ...
        {
            return new WORLDVIEWPROJECTION変数( Object );
        }
        ...
        public override void 変数を更新する( EffectVariable 変数, 変数更新時引数 引数 )
        {
            ...
            {
                case Object種別.カメラ:
                    行列を登録する( RenderContext.Instance.行列管理.ワールドビュー射影行列を作成する( 引数.モデル ), 変数 );
                    break;

...
namespace MMF.物理演算
{
	internal class 拘束ファクトリ : IDisposable
	{
        ...
        public void Dispose()
        {
            for( int i = _dynamicsWorld.NumConstraints - 1; i >= 0; --i )
            {
                var constraint = _dynamicsWorld.GetConstraint( i );
		...
		{
			_dynamicsWorld.AddConstraint( new Point2PointConstraint( 剛体, 拘束する点.ToBulletSharp() ) );
		}

		public void 剛体と剛体の間に点拘束を追加する( RigidBody 剛体A, RigidBody 剛体B, ref Vector3 剛体Aから見た拘束点の位置, ref Vector3 剛体Bから見た拘束点の位置 )
		{
			_dynamicsWorld.AddConstraint( new Point2PointConstraint( 剛体A, 剛体B, 剛体Aから見た拘束点の位置.ToBulletSharp(), 剛体Bから見た拘束点の位置.ToBulletSharp() ) );
		}
        ...
        private void 拘束にある一つの自由度へのバネを設定する( float バネの値, int 自由度の種類, Generic6DofSpringConstraint 拘束 )
		{
			if( バネの値 == 0.0f )
                return;

...
namespace MikuMikuFlex.Utility
{
	...
	{
		static public ShaderResourceView FromStream( Device device, Stream stream )
		{
			...
			using( var image = new System.Drawing.Bitmap( stream ) )
			{
				...
				using( var bitmap = image.Clone( imageRect, System.Drawing.Imaging.PixelFormat.Format32bppArgb ) )
				{
					...
					using( var texture = new Texture2D( device, textureDesc, dataBox ) )
					{
		...
		static public ShaderResourceView FromFile( Device device, string path )
		{
			var srv = (SharpDX.Direct3D11.ShaderResourceView) null;
